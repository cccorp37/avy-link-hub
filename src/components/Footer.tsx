      ...
      <div className="container mx-auto px-4 pt-24 pb-8">
        ...
        >
          ...
          <div className="md:col-span-4 space-y-5">
            <div className="flex items-center gap-2.5">
              <img src={avylinkLogo} alt="AvyLink Logo" className="w-10 h-10 rounded-xl object-cover" />
              <span className="font-dm font-black text-2xl text-white">
                Avy<span className="text-primary">Link</span>
              </span>
            </div>
            <p className="text-white/40 text-sm leading-relaxed max-w-xs">
              La plateforme de portfolio personnalisable conçue pour les créateurs africains et du monde entier.
            </p>
            <div className="flex gap-2">
              {[
                ...
                <a key={label} href="#" className="w-10 h-10 rounded-xl bg-white/[0.04] border border-white/[0.06] flex items-center justify-center text-white/40 hover:text-primary hover:bg-white/[0.08] transition-all">
          ...
          {/* Links */}
          {[
            { title: "Produit", links: ["Fonctionnalités", "Tarifs", "Templates", "Démo live", "Changelog"] },
            { title: "Ressources", links: ["Documentation", "Blog", "Tutoriels vidéo", "API", "Statut"] },
            { title: "Entreprise", links: ["À propos", "Contact", "Partenaires", "Mentions légales", "Confidentialité"] },
          ].map((col) => (
            <div key={col.title} className="md:col-span-2 space-y-4">
              <h4 className="font-dm font-bold text-white/60 text-[11px] uppercase tracking-[0.15em]">{col.title}</h4>
              <ul className="space-y-2.5">
                {col.links.map((link) => (
                  <li key={link}>
                    <a href="#" className="text-white/35 hover:text-white text-sm transition-colors">{link}</a>
                  </li>
                ))}
              </ul>
            </div>
        ...
        >
          <div className="flex-1">
            ...
            <p className="text-white/35 text-sm">Nouveautés, astuces et offres exclusives.</p>
          </div>
        ...
        <div className="border-t border-white/[0.04] pt-6 flex flex-col sm:flex-row items-center justify-between gap-3">
          <p className="text-white/25 text-sm">© 2026 AvyLink — Fait avec 💙 en Afrique</p>
