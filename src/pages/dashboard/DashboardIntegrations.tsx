...
const CATEGORIES = ["Tous", "Analytics", "Pixels", "Communication", "Musique", "Vidéo", "Paiement"];
...
export default function DashboardIntegrations({ profile, onUpdate }: Props) {
  ...
  const getRecord = (integrationId: string) => integrations.find(i => i.integration_id === integrationId);
  const isConnected = (integrationId: string) => !!getRecord(integrationId)?.is_connected;
  ...
  const openConfig = (def: IntegrationDef) => {
    ...
    def.configFields.forEach(f => {
      if (existing?.config[f.key]) {
        ...
        initialConfig[f.key] = (profile as Record<string, unknown>)?.[def.profileField] as string || "";
      } else {
        initialConfig[f.key] = "";
      }
    });
  ...
  const saveConfig = async (def: IntegrationDef) => {
    ...
    try {
      ...
      if (def.profileField && def.configFields[0]) {
        ...
        await onUpdate({ [def.profileField]: value || null } as Partial<Profile>);
      }
      ...
      if (existing) {
        ...
        setIntegrations(prev => prev.map(i => i.id === existing.id ? { ...i, is_connected: hasValues, config: configForm } : i));
      } else if (hasValues) {
      ...
      toast({ title: hasValues ? `✅ ${def.name} connecté !` : `${def.name} déconnecté` });
      setConfiguring(null);
    } catch (err) {
  ...
  const disconnect = async (def: IntegrationDef) => {
    const existing = getRecord(def.id);
    if (!existing) return;
    await supabase.from("integrations").update({ is_connected: false, config: {} }).eq("id", existing.id);
    if (def.profileField) {
      await onUpdate({ [def.profileField]: null } as Partial<Profile>);
    }
    setIntegrations(prev => prev.map(i => i.id === existing.id ? { ...i, is_connected: false, config: {} } : i));
    ...
    <div className="p-4 md:p-6 max-w-4xl mx-auto space-y-5">
                ...
                >
                  ...
                  <div className="p-4">
                    ...
                    <div className="flex gap-2">
                      <motion.button
                        whileTap={{ scale: 0.97 }}
                        onClick={() => connected ? openConfig(integration) : openConfig(integration)}
            ...
            >
              ...
              >
                <div className="flex items-center justify-between p-5 border-b border-border sticky top-0 bg-card z-10">
                  ...
                  <button onClick={() => setConfiguring(null)} className="p-1.5 rounded-lg hover:bg-secondary">
                    <X className="w-5 h-5" />
                  </button>
                </div>
                ...
                <div className="p-5 space-y-4">
                  ...
                  {connected && (
                    <div className="flex items-center gap-2 p-3 bg-green-50 rounded-xl border border-green-200">
                      ...
                      <span className="text-sm text-green-700 font-medium">Intégration active</span>
                    </div>
                  )}
                </div>
                ...
                <div className="p-5 border-t border-border flex gap-3">
                  ...
                  {connected && (
                    <Button variant="outline" onClick={() => disconnect(def)} className="rounded-xl text-red-600 border-red-200 hover:bg-red-50">
                      Déconnecter
                    </Button>
                  )}
                </div>
      ...
      >
        ...
        <div className="flex items-start gap-4 relative z-10">
          ...
          <div>
            <h3 className="font-dm font-semibold text-sm text-foreground mb-1">Comment ça marche ?</h3>
            <p className="text-xs text-muted-foreground leading-relaxed">
              Cliquez sur <strong>"Connecter"</strong> pour configurer une intégration. Renseignez vos identifiants (Pixel ID, URL, etc.) 
              et ils seront automatiquement injectés sur votre page publique. Les pixels de tracking se déclenchent à chaque visite.
            </p>
          </div>
        </div>
      </motion.div>
