...
import { Eye, MousePointerClick, Users, TrendingUp, Calendar, Activity, ArrowUp, BarChart3 } from "lucide-react";
...
export default function DashboardAnalytics({ profile }: Props) {
  ...
  const statCards = [
    { icon: Eye, label: "Vues", value: loading ? "…" : totalViews, change: `${period}j`, gradient: "var(--gradient-cta)" },
    { icon: Users, label: "Visiteurs", value: loading ? "…" : uniqueVisitors, change: `${period}j`, gradient: "var(--gradient-primary)" },
    { icon: MousePointerClick, label: "Clics", value: loading ? "…" : totalClicks, change: "Total", gradient: "var(--gradient-rose)" },
    { icon: TrendingUp, label: "CTR", value: loading ? "…" : `${ctr}%`, change: "Global", gradient: "var(--gradient-cta)" },
  ];
    ...
    <div className="p-4 md:p-6 max-w-4xl mx-auto space-y-5">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 md:gap-4">
        {statCards.map((stat, i) => (
          ...
          >
            <div className="absolute top-0 right-0 w-20 h-20 rounded-full opacity-[0.04] -translate-y-1/3 translate-x-1/3"
              style={{ background: stat.gradient }} />
            <div className="flex items-center justify-between mb-2">
              <div className="w-9 h-9 rounded-xl flex items-center justify-center shadow-sm" style={{ background: stat.gradient }}>
      ...
      >
        <div className="flex items-center gap-2.5 mb-4">
          ...
          <h3 className="font-dm font-semibold text-sm text-foreground">Vues & Clics</h3>
          <div className="ml-auto flex items-center gap-3 text-[10px]">
            <span className="flex items-center gap-1"><span className="w-3 h-1.5 rounded-full bg-primary inline-block" /> Vues</span>
            <span className="flex items-center gap-1"><span className="w-3 h-1.5 rounded-full inline-block" style={{ background: "hsl(var(--rose))" }} /> Clics</span>
          </div>
        </div>
        {loading ? (
            ...
            <AreaChart data={chartData}>
              <defs>
                <linearGradient id="analyticsViews" x1="0" y1="0" x2="0" y2="1">
                  <stop offset="5%" stopColor="hsl(204, 94%, 52%)" stopOpacity={0.15} />
                  <stop offset="95%" stopColor="hsl(204, 94%, 52%)" stopOpacity={0} />
                </linearGradient>
                <linearGradient id="analyticsClicks" x1="0" y1="0" x2="0" y2="1">
                  <stop offset="5%" stopColor="hsl(338, 85%, 65%)" stopOpacity={0.1} />
                  <stop offset="95%" stopColor="hsl(338, 85%, 65%)" stopOpacity={0} />
                </linearGradient>
              </defs>
              ...
              />
              <Area type="monotone" dataKey="views" stroke="hsl(var(--primary))" strokeWidth={2.5} fill="url(#analyticsViews)" name="Vues" />
              <Area type="monotone" dataKey="clicks" stroke="hsl(var(--rose))" strokeWidth={2} fill="url(#analyticsClicks)" strokeDasharray="4 2" name="Clics" />
            </AreaChart>
          </ResponsiveContainer>
        )}
      </motion.div>
      ...
      >
        <div className="flex items-center gap-2.5 mb-4">
          ...
          <h3 className="font-dm font-semibold text-sm text-foreground">Performance des liens</h3>
        </div>
        {loading ? (
                ...
                >
                  <div className="flex items-center justify-between text-sm">
                    <div className="flex items-center gap-2.5 min-w-0">
                      ...
                      <span className="font-medium text-foreground truncate group-hover:text-primary transition-colors">{link.title}</span>
                    </div>
      ...
      >
        <div className="flex items-center gap-2.5 mb-4">
          ...
          <h3 className="font-dm font-semibold text-sm text-foreground">Vues quotidiennes</h3>
        </div>
        {loading ? (
            ...
            <BarChart data={chartData} barSize={20}>
              <defs>
                <linearGradient id="barGradient" x1="0" y1="0" x2="0" y2="1">
                  <stop offset="0%" stopColor="hsl(204, 94%, 52%)" stopOpacity={1} />
                  <stop offset="100%" stopColor="hsl(207, 89%, 42%)" stopOpacity={1} />
                </linearGradient>
              </defs>
              <CartesianGrid strokeDasharray="3 3" stroke="hsl(var(--border))" vertical={false} />
              <XAxis dataKey="day" tick={{ fontSize: 11, fill: "hsl(var(--muted-foreground))" }} axisLine={false} tickLine={false} />
              ...
              />
              <Bar dataKey="views" fill="url(#barGradient)" radius={[8, 8, 0, 0]} name="Vues" />
            </BarChart>
          </ResponsiveContainer>
        )}
      </motion.div>
