...
import {
  BookOpen, Rocket, Link2, Palette, BarChart3, ShoppingBag, Users, Sparkles,
  Search, ChevronRight, PlayCircle, Lightbulb, ShieldCheck, CreditCard, Layers, MessageSquare,
} from "lucide-react";
...
const TUTORIALS: Tutorial[] = [
  {
    icon: Rocket, category: "Démarrage", duration: "2 min",
  ...
  {
    icon: Link2, category: "Liens", duration: "1 min",
  ...
  {
    icon: Palette, category: "Design", duration: "3 min",
  ...
  {
    icon: Layers, category: "Multi-pages", duration: "2 min",
  ...
  {
    icon: ShoppingBag, category: "Boutique", duration: "4 min",
  ...
  {
    icon: BarChart3, category: "Stats", duration: "1 min",
  ...
  {
    icon: Users, category: "Équipe", duration: "2 min",
  ...
  {
    icon: CreditCard, category: "Abonnement", duration: "1 min",
  ...
  {
    icon: ShieldCheck, category: "Sécurité", duration: "1 min",
    ...
    <div className="p-4 md:p-6 max-w-3xl mx-auto space-y-5">
      ...
      <motion.div custom={0} variants={fadeUp} initial="hidden" animate="visible"
        ...
        <div className="relative">
          <div className="flex items-center gap-3 mb-3">
            ...
            <div>
              <h1 className="font-dm font-bold text-xl md:text-2xl text-foreground">Comment utiliser AvyLink</h1>
              <p className="text-xs md:text-sm text-muted-foreground">Tutoriels, astuces et bonnes pratiques pour aller plus loin.</p>
            </div>
          </div>
      ...
      <motion.div custom={1} variants={fadeUp} initial="hidden" animate="visible"
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-sm text-foreground leading-snug">{TIPS[new Date().getDate() % TIPS.length]}</p>
        </div>
      </motion.div>
      ...
      <motion.div custom={3} variants={fadeUp} initial="hidden" animate="visible"
        ...
        <div className="px-5 py-3 border-b border-border/50 flex items-center justify-between">
          <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">Tutoriels ({filtered.length})</p>
          <PlayCircle className="w-4 h-4 text-muted-foreground" strokeWidth={1.8} />
        </div>
        ...
        {filtered.length === 0 ? (
          ...
          return (
            <div key={tuto.title} className="border-b border-border/30 last:border-0">
              <button
              ...
              >
                <div className="w-9 h-9 rounded-xl bg-gradient-to-br from-primary/15 to-primary/5 flex items-center justify-center flex-shrink-0">
                  <tuto.icon className="w-4 h-4 text-primary" strokeWidth={1.8} />
                </div>
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-0.5 flex-wrap">
                    <span className="px-1.5 py-0.5 rounded-md bg-secondary text-[10px] font-semibold text-muted-foreground">{tuto.category}</span>
                    <span className="text-[10px] text-muted-foreground">{tuto.duration}</span>
                  </div>
                  <p className="text-sm font-semibold text-foreground leading-snug">{tuto.title}</p>
                </div>
                <ChevronRight className={`w-4 h-4 text-muted-foreground flex-shrink-0 transition-transform ${isOpen ? "rotate-90" : ""}`} strokeWidth={1.8} />
              </button>
              {isOpen && (
                <motion.ol
      ...
      <motion.div custom={4} variants={fadeUp} initial="hidden" animate="visible"
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-xs text-muted-foreground">Notre équipe répond en moins de 24h.</p>
        </div>
        <button
