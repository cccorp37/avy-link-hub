...
import { Loader2, LayoutDashboard, Users, Link2, BarChart3, Shield, LogOut, Bell, Wrench, MessageSquare, FileCode, ArrowLeft } from "lucide-react";
      ...
      <aside className="hidden md:flex flex-col w-60 bg-card border-r border-border flex-shrink-0">
        <div className="flex items-center gap-2.5 px-4 h-14 border-b border-border">
          ...
          <div>
            ...
            <span className="ml-2 px-1.5 py-0.5 text-[10px] font-bold rounded bg-destructive text-destructive-foreground uppercase tracking-wider">Admin</span>
          </div>
        </div>

        {/* Back to dashboard */}
        <NavLink to="/dashboard" className="flex items-center gap-2 px-5 py-3 text-xs text-muted-foreground hover:text-primary transition-colors border-b border-border/50">
            ...
            <NavLink key={item.to} to={item.to} end={item.end}
              className={({ isActive }) =>
                `flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all border-l-[3px] ${
                  isActive ? "bg-primary/10 text-primary border-primary" : "text-muted-foreground hover:bg-secondary hover:text-foreground border-transparent"
                }`
              }>
        ...
        <div className="border-t border-border px-3 py-3">
          <button onClick={async () => { await signOut(); navigate("/"); }} className="w-full flex items-center gap-2 px-3 py-2 rounded-xl text-sm text-muted-foreground hover:text-destructive hover:bg-destructive/10 transition-colors">
      ...
      <div className="flex-1 flex flex-col min-h-screen overflow-hidden">
        <header className="h-14 px-6 flex items-center justify-between border-b border-border bg-card/80 backdrop-blur-sm flex-shrink-0">
          ...
          <div className="flex items-center gap-2">
            <NavLink to="/dashboard" className="px-3 py-1.5 text-xs rounded-lg border border-border text-muted-foreground hover:text-foreground transition-colors hidden md:block">
              ← Dashboard
            </NavLink>
            <span className="px-2.5 py-1 text-xs font-bold rounded-full gradient-cta text-primary-foreground">Administrateur</span>
          </div>
        </header>
